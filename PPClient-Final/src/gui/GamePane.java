...
public class GamePane extends JPanel {
	...
	public GamePane(boolean isModerator, GameClient client) {
		...
		questionLabel = DefaultComponentFactory.getInstance().createLabel("Question description");
		...
		JLabel lblNewJgoodiesLabel_2 = DefaultComponentFactory.getInstance().createLabel("Results of question");
		lblNewJgoodiesLabel_2.setBounds(287, 59, 150, 14);
		...
		NewScroll.setBounds(287, 95, 158, 267);
		add(NewScroll);
		
		resultOfQuestion = new JTextPane();
		resultOfQuestion.setEditable(false);
		
		NewScroll.setViewportView(resultOfQuestion);
		...
		if(isModerator){
		 ...
		 nextRound.addMouseListener(new MouseAdapter() {
			...
			nextRound.setBounds(316, 411, 140, 23);
			add(nextRound);
		}
